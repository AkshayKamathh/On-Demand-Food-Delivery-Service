...
export default function SearchBar({
  ...
  // Auto-update whenever query or filters change
  useEffect(() => {
        ...
        <div className="fixed right-4 top-20 z-50 w-64 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 shadow-2xl max-h-64 overflow-y-auto">
          <div className="space-y-2">
            {categories.map((category) => (
              <label
                key={category.value}
                className="flex items-center gap-3 p-2.5 hover:bg-zinc-50 dark:hover:bg-zinc-700 rounded-lg cursor-pointer text-sm"
              >
                <input
                  type="checkbox"
